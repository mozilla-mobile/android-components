 ...
 */
internal class PlacesStorageMaintenanceWorker(context: Context, params: WorkerParameters) :
    StorageMaintenanceWorker(context, params) {

    val logger = Logger(PLACES_STORAGE_MAINTENANCE_WORKER_TAG)
    ...
    companion object {
        ...
        internal const val DB_SIZE_LIMIT_IN_BYTES =
            75 * 1024 * 1024 // corresponds to 75MiB (in bytes)
        internal const val WORKER_PERIOD_IN_HOURS = 24L
        internal const val UNIQUE_NAME = "$IDENTIFIER_PREFIX.PlacesStorageMaintenanceWorker"
        internal const val PLACES_STORAGE_MAINTENANCE_WORKER_TAG =
            "$IDENTIFIER_PREFIX.PlacesStorageMaintenanceWorker"
    }
}
