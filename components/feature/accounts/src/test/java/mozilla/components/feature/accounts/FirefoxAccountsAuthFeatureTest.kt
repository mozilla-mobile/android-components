...
class FirefoxAccountsAuthFeatureTest {
    ...
    fun `auth interceptor`() {
        ...
        // Non-final FxA url.
        assertNull(feature.interceptor.onLoadRequest(mock(), false, "", "https://accounts.firefox.com/not/the/right/url", false))
        verify(manager, never()).finishAuthenticationAsync(any())

        // Non-FxA url.
        assertNull(feature.interceptor.onLoadRequest(mock(), false, "", "https://www.wikipedia.org", false))
        verify(manager, never()).finishAuthenticationAsync(any())

        // Redirect url, without code/state.
        assertNull(feature.interceptor.onLoadRequest(mock(), false, "", "https://accounts.firefox.com/oauth/success/123/", false))
        verify(manager, never()).finishAuthenticationAsync(any())

        // Redirect url, without code/state.
        assertNull(feature.interceptor.onLoadRequest(mock(), false, "", "https://accounts.firefox.com/oauth/success/123/test", false))
        verify(manager, never()).finishAuthenticationAsync(any())

        // Code+state, no action.
        assertEquals(
            RequestInterceptor.InterceptionResponse.Url(redirectUrl),
            feature.interceptor.onLoadRequest(mock(), false, "", "https://accounts.firefox.com/oauth/success/123/?code=testCode1&state=testState1", false)
        )
        ...
        assertEquals(
            RequestInterceptor.InterceptionResponse.Url(redirectUrl),
            feature.interceptor.onLoadRequest(mock(), false, "", "https://accounts.firefox.com/oauth/success/123/?code=testCode2&state=testState2&action=signin", false)
        )
        verify(manager).finishAuthenticationAsync(
        ...
        assertEquals(
            RequestInterceptor.InterceptionResponse.Url(redirectUrl),
            feature.interceptor.onLoadRequest(mock(), false, "", "https://accounts.firefox.com/oauth/success/123/?code=testCode3&state=testState3&action=signup", false)
        )
        verify(manager).finishAuthenticationAsync(
        ...
        assertEquals(
            RequestInterceptor.InterceptionResponse.Url(redirectUrl),
            feature.interceptor.onLoadRequest(mock(), false, "", "https://accounts.firefox.com/oauth/success/123/?code=testCode4&state=testState4&action=pairing", false)
        )
        verify(manager).finishAuthenticationAsync(
        ...
        assertEquals(
            RequestInterceptor.InterceptionResponse.Url(redirectUrl),
            feature.interceptor.onLoadRequest(mock(), false, "", "https://accounts.firefox.com/oauth/success/123/?code=testCode5&state=testState5&action=someNewActionType", false)
        )
        verify(manager).finishAuthenticationAsync(
