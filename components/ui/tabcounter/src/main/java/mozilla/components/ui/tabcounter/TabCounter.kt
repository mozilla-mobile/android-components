...
class TabCounter @JvmOverloads constructor(
    ...
    internal var currentRatio: Float = 0.toFloat()
    ...
    private fun createBarAnimatorSet(animatorSet: AnimatorSet) {
        ...
        val scaleUp1 = ObjectAnimator.ofFloat(box, "scaleX",
    ...
    private fun formatForDisplay(count: Int): String {
        return if (count > MAX_VISIBLE_TABS) {
            text.setPadding(0, 0, 0, INFINITE_CHAR_PADDING_BOTTOM)
            SO_MANY_TABS_OPEN
        } else NumberFormat.getInstance().format(count.toLong())
    }

    private fun adjustTextSize(newCount: Int) {
        currentRatio = if (newCount in TWO_DIGITS_TAB_COUNT_THRESHOLD..MAX_VISIBLE_TABS) {
        ...
        val textSize = currentRatio * counterBoxWidth
