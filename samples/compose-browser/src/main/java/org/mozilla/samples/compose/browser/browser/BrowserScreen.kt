...
import mozilla.components.lib.state.ext.scopedSaveableStore
...
fun BrowserScreen(navController: NavController) {
    ...
    val editState = store.observeAsState { state -> state.editMode }
    val editUrl = store.observeAsState { state -> state.editText }
